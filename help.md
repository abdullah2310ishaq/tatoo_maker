  ...
  Widget _buildMainImage(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final top = (constraints.maxHeight * 0.14).clamp(24.0, 220.0);
        ...
        return Stack(
          children: [
            Positioned.fill(
              child: Builder(
                builder: (context) {
                  ...
                  // Flower module rule:
                  // - non-PRO: lightly blur result
                  // - locked (limit reached): heavily blur + show locked overlay card
                  if (isProUnlocked) return image;

                  final sigma = isLocked ? 14.0 : 4.0;
                  final dimOpacity = isLocked ? 0.18 : 0.08;
            ...
            if (isLocked) _buildLockedOverlayCard(),
          ],
        );
      },
    );
